...
* 可为每一个仓库分类分配充满活力的色彩标签，枯燥的账号管理也能赏心悦目。
* 抽屉式「新增账号」页面，上划调出，下划收起。
* 隐藏式「关于信息」: 设置页底部注脚处双击唤出。
* 发布默认缩放比83%，同时提供86%和89%版本，作为适合中年用户使用的人性化优化。(请按照缩放比增序尝试后再投入使用，否则回退不支持直接更新，需卸载重新安装)
* 更多细节和功能等待你的发现！

⚠️⚠️⚠️ **App默认PIN码:1234** ⚠️⚠️⚠️
_____________________________________________________
## 🛠 Tech Stack Info
...
本项目（La Clave App）仅作为个人技术学习与研究之用，不提供任何形式的商业级担保。
您的所有数据均由动态 AES 算法加密并严格保留在您的设备本地。但请注意：
1. **请务必定期使用内置的「加密导出」功能备份您的数据。**
2. 开发者不对因设备损坏、误卸载 App、忘记主密码或不可抗力导致的数据丢失承担任何连带责任。
3. 覆盖安装升级是安全的，**请仅在版本号冲突的情况下卸载重装，备份数据不受影响**。
